...
| Token                       | Typical use                           |
| --------------------------- | ------------------------------------- |
| **`--m__color-red-600`**     | Alerts, deepfake, medical/STT accents |
| **`--m__color-yellow-500`**  | Music-related accents                 |
| **`--m__color-purple-500`**  | Brand accents                         |

In a colored source SVG:

- **`fill="var(--m__color-red-600)"`** (or any palette token) — fixed accent; does not follow parent `color`
- **`fill="currentColor"`** — monochrome part; inherits `color` from where the icon is used

Example:

```xml
<rect x="4.975" y="4.975" width="4.727" height="3.708" fill="var(--m__color-red-600)" />
...
3. For colored product icons, map editor fills to a palette token (e.g. **`var(--m__color-red-600)`**) or **`currentColor`** as above; add the filename to **`COLORED_ICONS`** if needed.
...
- **`deepfake.svg`** → **`#deepfake`**: colored icon — top bars use **`var(--m__color-red-600)`**; bottom bars use **`currentColor`**. Listed in **`COLORED_ICONS`**.
- **`stt-med.svg`** → **`#stt-med`**: colored icon — all bars use **`var(--m__color-red-600)`**. Listed in **`COLORED_ICONS`**.
- **`velma.svg`** → **`#velma`**: colored icon — accent shapes use **`var(--m__color-purple-500)`**; other shapes use **`currentColor`**. Listed in **`COLORED_ICONS`**.
