...
Layout: sales default — the upload plate collapsed (`uploadPlateState 'uploaded'`, canonical expand; the "Try demo audio" split menu with reference-tagged recordings lives in the expanded state), the demo recording already analyzed. Sidebar: hard-enabled "STT+LLM comparison" checkbox; a stack select ("Whisper v4 + GPT-5.5") opening a dialog with STT and LLM radio lists (checkmark rows, `.vlb-radio`) and the emotions checkbox — the checkbox really flips the page (class `vlb-emotions-on` on `.dashboard-layout`); the "Set up behaviors" link (future popover) moved here from the plate header; the behaviors multi-select (3 of 8 dashboard-library names). Player: lane 1 Velma with emotion clips and kikis; lane 2 the stack — two alternating grays mixed from `--m__text` (deliberately not `emotion-neutral`, so a genuinely neutral Velma clip stays unique; color means nothing, the clip pattern survives), or the translucent guessed palette (`rgba(var(--gec), .45)` = "assumed, not heard") when emotions are on, with two divergences from Velma (u3, u7 — the emotion lived in the voice). Per-behavior blocks are the same fingerprint design (`.vlb-mini` wrapping `pg-player-dataviz` with `--player-visualization-height: 100%`): full clip row on both lanes, non-signal clips in the muted grays, detections colored (Velma) / darker gray or guessed (stack), false positives red-600, misses as ghost kikis (outline-only star). Every kiki is a `data-modal-open` entry into its receipt modal: agent tag, confidence + time, reasoning, a 1–2 utterance excerpt with the evidence line highlighted (blue tint; red for the false alarm), and the outcome line (green correct / red false). A caption row under the player names the recording and opens the wide transcript modal (canonical `pg-transcript-utterance` list with `ec-*` emotions and `m__behavior-link`s). Dashboard (last floor, 50/50): a pure-CSS precision×recall scatter with the green efficient-corner tint and two labeled dots, and on the right a 2×3 card pile (detection accuracy, false alarms, cost per audio hour, median time to signal, confidence when flagging, signals found) plus a mini scorecard table. All positions are hand-computed percentages from the Harborview call in `velma-fraud-config.js` (68 440 ms). The sidebar panel is rounded by hand, not via `.m__rounded`, whose overflow clip would cut the dropdowns. Headless-screenshot caveat: toggling the theme class after load freezes `transition: color` mid-flight under `--virtual-time-budget` — set the theme via localStorage before first paint instead.
