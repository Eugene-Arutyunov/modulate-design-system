...
- `src/blog/icons.html` → `/blog/icons/` — September 8, 2026 — Five Requirements for Icons (each requirement carries an example block built on the Graphics → Icons tile grid: an Obvious / Not really pair of three-tile groups under “Convey meaning” (search, calendar, music / behaviors, api-key, deepfake), a row of six captioned tiles under “Form a system” (account, behaviors, batch, api-key, redaction, language) — both without captions; a No/Yes comparison under “Be distinguishable” — Document, Duplicate and Search as three stroke glyphs on one identical frame (the corner badge is a plus or a lens, readable only through the gap cut into the frame), drawn inline and marked `data-icon-copy="off"`, against three of ours; under “Integrate into text and interface” a static, link-less copy of the playground sidebar (no group titles, no AI Music, labels truncated) at one fifth (`--columns: 5`) next to a display-sized CoFo Gothic Medium paragraph with `#emotions` and `#behaviors` set inline at 1em (`.icon-example__prose`, four fifths via `--columns: 1.25`); the whole flat set (the design-system Graphics → Icons row minus the sign-in logos, `#id` labels, click-to-copy) closes “Integrate” as “our current set”; a 3D row of eleven (velma, transcript, deepfake, redaction, music, stt-med, language, emotions, voice-match, behaviors, modulate) closes “Convey the product’s image”, followed by an Icon Studio screenshot (`assets/images/icon-studio.png`) as a card-styled link (`a.post-figure-link` in `blog.css`, the home promo card's recipe) and a one-line “Try it”. Wrapper `.icon-example` in `blog.css`)
...
**Scripts:** `src/assets/service/docs-icon-copy.js` — click on a Graphics → Icons tile copies a standalone SVG (resolved fills) to the clipboard; tiles marked `data-icon-copy="off"` are skipped (and styled non-interactive) for icons that are not in the sprite.  
