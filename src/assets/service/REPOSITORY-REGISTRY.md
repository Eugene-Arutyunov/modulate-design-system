...
Layout: sales default, no site header (`hideSiteHeader` flag in `service/layout.html`) and no page h1 — the page opens with the collapsed upload plate at canonical heights (no meta counter — `uploadPlateShowMeta = false`; canonical processing animation trimmed to four stages: Transcript → Speakers → Emotions → Behaviors; the “Stream demo” split menu lists three recordings — the menu opens inside the overflow-clipped plate, and three is what reliably fits at the canonical plate height). Sidebar: Behaviors group first (kiki glyph in the title, the multi-select dropdown), then STT + LLM — two native label-less `m__select S` pickers (canon: no `__control` wrapper — that wrapper is only for labeled selects, otherwise the box draws twice), a “System prompt” outline button opening the prompt modal (`modal-behaviors-edit` canon: autogrow textarea with the full invented prompt incl. `{{stt_model}}`/`{{selected_behaviors}}` placeholders, Restore default / Cancel / Save footer), and the “Guess emotions” checkbox (re-renders the shared and signal players live). Per-behavior blocks are headed by plain h3 (no kiki links). The Transcript outline button lives inside the shared player's chrome on the hover-transcript line's spot (JS re-adopts it after every render; it fades while the line is visible). The playground upload plate no longer carries a `margin-top` — the header gap is an explicit `m__space S` in `playground-layout.html`. Both the shared player and the per-behavior players (“плеер с выбранным сигналом”) are rendered by `fingerprint.js` in the technology-comparison variant (see the Modulate Fingerprint tool section): agent lanes with genuinely different segmentation per lane (Velma — 9 utterance clips with heard emotions, the stack — 16 finer gapless VAD chunks with STT-flavored lowercase texts, varied slicing — threes, twos, whole utterances with shifted bounds, one chunk swallowing the next utterance's head — and three emotion divergences when guessing is on), per-lane hover (`laneHover`), kikis at the signal times (`behaviourAtSec`), every kiki a `data-modal-open` entry into its receipt modal (canonical `modal-behaviors-*` markup + `assets/prototypes/modals.js`), hover captions and clip hovers canonical. The stack lane paints by one rule everywhere (shared player with emotions off and signal players alike): `clip-quiet-tech` base, `clip-hit` light-gray highlight on every glyph-carrying chunk (false positives keep the red kiki but a gray fill; ghosts highlight too), and with “Guess emotions” on the highlight becomes the translucent guessed-emotion color (`clip-guess`); the toggle re-renders the shared player **and** the signal players. The Transcript button opens the two-column transcript modal (`vlb-transcript-duo`: Velma's diarized utterances left, the stack's finer undiarized chunks right, 64rem modal); after the strips comes the dashboard: a pure-CSS precision×recall scatter — efficient corner as a blurred gradient blob (the scatterplot tool's highlight-zone recipe) plus four quadrant captions in the sidebar-subtitle style (trustworthy-but-missing / everything-and-true / missing-and-noisy / everything-but-drowning) — and the metric pile (2×3 cards: detection accuracy, false alarms, cost per audio hour, median time to signal, confidence when flagging, signals found; plus a mini scorecard table). Degradation rule: external uploads keep cost / speed / signal counts only. Headless-screenshot caveat: toggling the theme class after load freezes `transition: color` mid-flight under `--virtual-time-budget` — set the theme via localStorage before first paint instead.
