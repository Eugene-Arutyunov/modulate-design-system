...
- Метр (четвёртый раунд правок 19.08): вертикальная колонка `vf-risk` — третья, самая узкая колонка нижней сетки (1/2 + 1/3 + 1/6). Заливка снизу вверх в цвете состояния (caption → warn-оранжевый → error), порог — полупрозрачная dashed-линия, eyebrow «Fraud risk» всегда сверху, живой процент в середине, внизу вердикт-строка «Account takeover pattern · 94%» и компактные экшен-теги «Suspected fraud» / «Manual review» (config сокращён до двух экшенов). За порогом весь виджет краснеет: `vf-critical` на корне → `color-mix(error 20%, bg)`. Старые `vf-meter*`, вердикт-стейтмент `pg-verdict-statement` и горизонтальный бар удалены. Скрытые вердикт/экшены не резервируют место (`display: none` + `@starting-style`).
