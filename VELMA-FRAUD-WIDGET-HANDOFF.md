...
Смоук: play → клипы/реплики/сигналы появляются, ленты держат последнюю запись у нижней кромки (автоскролл rect-based), метр растёт, на 0:56 — вердикт-стейтмент и экшены (до этого места под метром нет); сик перетаскиванием пересчитывает всё из t. Колёсико над лентами заблокировано — скролл лент управляется позицией курсора по Y (`bindHoverScroll`; при ховере автоскролл уступает). Статус-тега аудио больше нет: mp3 подхватывается молча, без него — симулированный клок.
