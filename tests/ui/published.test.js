...
test('clean checkout renders reviewed public captures and copies only referenced images', () => {
  ...
  try {
    ...
    fs.writeFileSync(path.join(root,'src/service/ui.yaml'),'current:\n  - {id: demo, route: /demo, sections: []}\ntarget:\n  - {id: demo, route: /demo/, sections: []}\n');
    const base = path.join(root,'ui-public');fs.mkdirSync(base);
    fs.writeFileSync(path.join(base,'demo.webp'),'reviewed pixels');
    fs.writeFileSync(path.join(base,'private.txt'),'must not ship');
    const report = {checks:[{pageId:'demo',scenario:'default',privacy:{version:1,reviewed:true},screenshots:{production:'/ui-audit/demo.webp'}}]};
    const result = JSON.stringify(report);
    fs.writeFileSync(path.join(base,'results.json'), result);
    fs.writeFileSync(path.join(base,'manifest.json'),JSON.stringify({version:1,results:digest(result),images:{'demo.webp':digest('reviewed pixels')}}));
    assert.match(renderUI('compare',root),/src="\/ui-audit\/demo.webp"/);
    publishAssets(root,path.join(root,'_site'));
    assert.deepEqual(fs.readdirSync(path.join(root,'_site/ui-audit')),['demo.webp']);
    fs.writeFileSync(path.join(base,'demo.webp'),'unreviewed replacement');
