...
(async () => {
  ...
  try {
    ...
    const ui = page.locator('#ui-structure');
    await expect(ui.getByRole('tab')).toHaveCount(2);
    await expect(ui.locator('.m__segmented-nav [role=tab]')).toHaveCount(2);
    await expect(ui.getByRole('tab', { name: 'Scheme', exact: true })).toHaveAttribute('aria-selected', 'true');
    ...
    await ui.getByRole('tab', { name: 'Compare', exact: true }).click();
    ...
    const tabStyle = await ui.getByRole('tab', { name: 'Compare', exact: true }).evaluate(tab => {
    ...
    await page.reload();
    await expect(ui.getByRole('tab', { name: 'Compare', exact: true })).toHaveAttribute('aria-selected', 'true');
