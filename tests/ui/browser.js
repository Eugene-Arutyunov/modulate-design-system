...
(async () => {
  ...
  try {
    ...
    await page.goto(`${base}/tools/ui-scheme/`);
    ...
    await expect(ui.locator('#ui-viz-panel tbody > tr').first()).toBeVisible();
    const schemeTabY = (await ui.locator('.ui-viz__tabs').boundingBox()).y;
    const ids = await ui.locator('#ui-viz-panel tbody > tr').evaluateAll(rows => rows.map(row => row.dataset.pageId));
    assert.ok(ids.includes('dashboard-home') && ids.includes('internal-users'));
    await expect(ui.locator('#ui-viz-panel thead th')).toHaveText(['Route', 'Prototype', 'Production']);
    ...
    await expect(page).toHaveURL(`${base}/tools/ui-compare/`);
    await expect(page.locator('header.header')).toHaveCount(0);
    ...
    await expect(ui.locator('#ui-viz-panel tbody > tr')).toHaveCount(ids.length);
    await page.goBack();
    await expect(page).toHaveURL(`${base}/tools/ui-scheme/`);
    await expect(ui.getByRole('link', { name: 'Scheme', exact: true })).toHaveAttribute('aria-current', 'page');
    await page.goForward();
    await expect(page).toHaveURL(`${base}/tools/ui-compare/`);
    await expect(ui.locator('#ui-viz-panel tbody > tr')).toHaveCount(ids.length);
    assert.deepEqual(await ui.locator('#ui-viz-panel tbody > tr').evaluateAll(rows => rows.map(row => row.dataset.pageId)), ids);
    await expect(ui.locator('#ui-viz-panel thead th')).toHaveText(['Route', 'Prototype', 'Production']);
