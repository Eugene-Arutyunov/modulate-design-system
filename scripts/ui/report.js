...
function exportReport({ data, report, latestDir, output }) {
  ...
  const recommendations = review.shared.length ? `<section class="recommendations"><h2>General recommendations</h2>
    <p>Repeated style differences across the captured screens. Fix these in shared components.</p>
    ${review.shared.map(group => `<article id="${escape(group.id)}"><h3>${escape(group.title)}</h3>
      <div class="style-values"><div><p class="meta">Prototype</p><code>${escape(group.property)}: ${escape(group.prototype)}</code></div><div><p class="meta">Production</p><code>${escape(group.property)}: ${escape(group.production)}</code></div></div>
      <p class="recommendation-fix"><strong>Recommendation:</strong> ${escape(group.recommendation)}</p>
      <ul>${group.affected.map(item => `<li><a href="#${escape(item.anchor)}">${escape(item.title)}${item.scenario !== 'default' ? ` · ${escape(item.scenario)}` : ''}</a> — ${escape(item.elements.join(', '))}</li>`).join('')}</ul>
...
.recommendations{margin-top:48px}.style-values{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:16px}.recommendation-fix{padding:16px;background:#f2f2f6;border-radius:8px}.differences{margin-top:16px}article,.state{scroll-margin-top:24px}
...
<main>${pages}${recommendations}</main><footer>Open the images to inspect them at full size. This folder contains the report and its comparison images.</footer></body></html>`;
