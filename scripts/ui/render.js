...
function notes(check, route, review, base) {
  ...
  return `<div class="ui-viz__differences" aria-label="Differences in production">${check.fingerprint === route.fingerprint && check.status === 'match' ? '<p class="caption">No differences detected.</p>' : ''}
...
function comparison(route, checks, review, base) {
  return (checks.length ? checks : [{scenario:'default',pageId:route.id}]).map(check=>`<div class="ui-viz__comparison-state" data-scenario="${escape(check.scenario)}" id="${escape(checkId(check))}">${['prototype','production'].map(side=>{
    ...
    return `<figure class="ui-viz__screenshot" data-side="${side}" data-label="${label}"><figcaption class="caption">${checks.length>1||check.scenario!=='default'?escape(check.scenario):''}</figcaption><div class="ui-viz__capture-url">${src ? address : empty}</div><div class="ui-viz__preview">${src?`<a class="ui-viz__image-frame" href="${escape(src)}" target="_blank" rel="noopener" title="Open full screenshot"><img src="${escape(src)}" alt="${escape(`${label}: ${route.title}, ${check.scenario}`)}" loading="lazy"></a>`:''}${side==='prototype'?`${src && check.fingerprint !== route.fingerprint ? '<p class="ui-viz__stale-notice">Prototype changed. Capture again to compare.</p>' : ''}${prototypeNotes(check)}`:(src?notes(check,route,review,base):'')}</div></figure>`;
  }).join('')}</div>`).join('');
}
function renderUI(view, root = process.cwd()) {
  ...
  const table = (rows, label) => `<table class="ui-viz__table ui-viz__table--${view}">${columns}${headers([label,'Prototype','Production'])}<tbody>${rows.map(route=>`<tr data-page-id="${escape(route.id)}"><td class="ui-viz__page-cell" data-label="Route"><code class="ui-viz__route-title">${route.target?.title_deprecated||route.current?.title_deprecated?`<del>${escape(route.target?.title_deprecated||route.current?.title_deprecated)}</del> `:''}${escape(route.title)}</code></td>${view==='compare'?`<td class="ui-viz__comparison-cell" colspan="2">${comparison(route,checks.filter(c=>c.pageId===route.id),review,base)}</td>`:`<td data-label="Prototype">${structure(route.target,'prototype')}</td><td data-label="Production">${structure(route.current,'production')}</td>`}</tr>`).join('')}</tbody></table>`;
  const pages = data.routes.filter(route => !isComponent(route));
  const components = data.routes.filter(isComponent);
  const navigation = `<nav class="ui-viz__section-nav" aria-label="Table of Contents"><p><strong>Table of Contents</strong></p><ul>${[...(view==='compare' && review.shared.length ? [['general-issues','Issues']] : []), ['pages','Pages'], ...(components.length ? [['components','Components']] : [])].map(([id,title]) => `<li><a href="#${id}">${title}</a></li>`).join('')}</ul></nav>`;
  return `${view==='compare'?navigation+issues(review):''}<section id="ui-viz-panel" aria-labelledby="ui-tab-${view}">${error}<div id="pages"><h2>Pages</h2></div>${table(pages,'Route')}${view==='compare'&&components.length?`<h2 id="components">Components</h2>${table(components,'Component')}`:''}</section>`;
}
